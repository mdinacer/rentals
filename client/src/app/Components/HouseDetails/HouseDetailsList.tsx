...
export default function HouseDetailsList({ details }: Props) {
  return (
    <div className='grid md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-y-3 flex-row flex-wrap gap-x-4'>
      {details.floors > 0 && (
        <ListItem icon={<FloorIcon />} title='Floors' value={details.floors} />
      )}
      {details.rooms > 0 && (
        <ListItem icon={<RoomIcon />} title='Rooms' value={details.rooms} />
      )}
      {details.beds > 0 && (
        <ListItem icon={<BedIcon />} title='Beds' value={details.beds} />
      )}
      {details.baths > 0 && (
        <ListItem icon={<BathIcon />} title='Baths' value={details.baths} />
      )}
      {details.kitchens > 0 && (
        <ListItem
          icon={<KitchenIcon />}
      ...
      {details.pools > 0 && (
        <ListItem icon={<PoolIcon />} title='Pools' value={details.pools} />
      )}
    </div>
  );
}
