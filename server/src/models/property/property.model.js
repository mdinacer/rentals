...
Fawn.init(process.env.MONGO_URL, 'tempPropertiesCollection');
...
async function DeleteProperty(user, propertyId) {
  ...
  Fawn.Task()
    .remove(Property, { _id: property._id })
    .remove('reviews', { property: property._id })
    .remove('rents', { property: property._id })
    .run();
}
